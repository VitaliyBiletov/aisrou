            ...
            return (
                <>
                    <div className='print__container'>
                        <button className='print__button' onClick={handlePrint}><FontAwesomeIcon icon={faPrint}/>
                        </button>
                    </div>
                    {data.map((item, index) =>
                        <>
...
function Status({...props}) {
    ...
    const setNextItem = (index, length) => {
        return index < length - 1 ? setActiveItem(index + 1) : setActiveItem(0)
    }
...
Task.propTypes = {
    ...
    instruction: PropTypes.oneOfType([PropTypes.string, PropTypes.array]),
