        ...
        {Object.keys(data).length !== 0 ?
            <div className="dynamic-data">
                <div className="dynamic-data__container">
                        ...
                        <div className='dynamic-data__section-container'>
                            {data.stateOfFunc.data.map((item, index) => {
                                return <div key={index}
                                            className={`dynamic-data__column dynamic-data__column_num_${index}`}>
                                    {Object.entries(item).map(([key, value], index) => <p key={index}>
                                        <b>{key}</b>: {value}</p>)}
                                </div>
                            })}
                        </div>
                    ...
                    <div className="dynamic-data__writing">
                        <h3 className='dynamic-data__section-title'>Письмо</h3>
                        <div className='dynamic-data__section-container'>
                            {data.writingResults.map((item, index) => {
                                return <div key={index}
                                            className={`dynamic-data__column dynamic-data__column_num_${index}`}>
                                            ...
                                            <div key={index}>
                                                ...
                                                <ul>
                                                    {Object.keys(translateTitle.writing[key].data).map((name, index) => {
                                                        return item.skills[key][name] ?
                                                            <li key={index}>{translateTitle.writing[key].data[name]}</li> : null
                                                    })}
                                                </ul>
                                            </div>
                                        )
